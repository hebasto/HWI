...
def backup_device_handler(args, client):
    return backup_device(client, args.label, args.backup_passphrase)

def displayaddress_handler(args, client):
    return displayaddress(client, args.path, args.sh_wpkh, args.wpkh)

def enumerate_handler(args):
    return enumerate(args.password)

def getmasterxpub_handler(args, client):
    return getmasterxpub(client)

def getxpub_handler(args, client):
    return getxpub(client, args.path)

def getkeypool_handler(args, client):
    return getkeypool(client, args.path, args.start, args.end, args.internal, args.keypool, args.account, args.sh_wpkh, args.wpkh)

def restore_device_handler(args, client):
    return restore_device(client, args.label)

def setup_device_handler(args, client):
    return setup_device(client, args.label, args.backup_passphrase)

def signmessage_handler(args, client):
    return signmessage(client, args.message, args.path)

def signtx_handler(args, client):
    return signtx(client, args.psbt)
